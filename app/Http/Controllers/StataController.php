...
/*
        ...
        // 개발서버 test시는 추가해주지 않으면 klips로 이동함 :  request()->getHttpHost()=='52.79.82.226'
...
/*            if(request('add_h')==null && request('add_p')==null && $addTxt == '') {
        ...
        if(Storage::disk('public')->exists('roundstata(dta)/'.$filename_req.'_'.$filenameAddDate.'.zip')) {
        //if(file_exists('stata/klips/'.$filename_req.'.zip') ) {
            $isSuccess = true;
            try {
                Storage::disk('public')->move('roundstata(dta)/'.$filename_req.'_'.$filenameAddDate.'.zip',
